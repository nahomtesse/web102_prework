...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        ...
        // append the game to the games-container
            document.getElementById('games-container').appendChild(gameInfo);
        }
...
const totalContributions = GAMES_JSON.reduce((p, n) => p + n.backers, 0);
...
const totalRaised = GAMES_JSON.reduce((p, n) => p + n.pledged, 0);
...
function filterUnfundedOnly() {
    ...
    const goal_unmet = GAMES_JSON.filter(game => game.pledged < game.goal);
    //console.log(goal_unmet);
...
function filterFundedOnly() {
    ...
    const goal_met = GAMES_JSON.filter(game => game.pledged >= game.goal);
...
const displayStr = `A total of ${"$" + totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${totalUnfunded} games remain unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container

const newElement = document.createElement('div');
newElement.innerHTML = `<p>${displayStr}</p>`

descriptionContainer.appendChild(newElement.firstChild);
...
firstGameContainer.appendChild(firstGameElement.firstChild);
